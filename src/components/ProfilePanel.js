...
import { isProfileDataLoadedSelector } from "../selectors/getProfileData";
import {
  ImageWrapper,
  NameWrapper,
  Name,
...
class ProfilePanel extends PureComponent {
  ...
  render() {
    const { isProfileDataLoaded, profileData } = this.props;
    return (
      <>
        {isProfileDataLoaded && (
          <>
            <Top>
              <ImageWrapper src={`${profileData.avatar_url}`} alt="avatar" />
              <NameWrapper>
            ...
            <Block>Block or report user</Block>
          </>
        )}
      </>
    );
...
function mapStateToProps(state) {
  return {
    profileData: state.getProfileData.data,
    isProfileDataLoaded: isProfileDataLoadedSelector(state)
  };
}
